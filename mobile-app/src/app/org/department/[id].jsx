...
import {
  useGetOrgDepartmentsQuery, useGetOrgUsersQuery,
  usePatchOrgDepartmentMutation, useDeleteOrgDepartmentMutation,
...
export default function OrgDepartmentDetailPage() {
  ...
  const { data: deptsData, isLoading, isFetching, refetch } = useGetOrgDepartmentsQuery();
  const { data: usersData } = useGetOrgUsersQuery(500, { skip: !canUpdateDepartments });
  const [patchDepartmentMutation] = usePatchOrgDepartmentMutation();
  const [deleteDepartmentMutation] = useDeleteOrgDepartmentMutation();

  const department = useMemo(() => deptsData?.items?.find(d => String(d.id) === String(departmentId)), [deptsData, departmentId]);
  ...
  useEffect(() => {
    if (department) {
      ...
      setForm({
        ...
        attendanceRadius: department.attendanceRadius ? String(department.attendanceRadius) : "25",
        isActive: department.isActive !== false,
        leaderId: department.leaderId ? String(department.leaderId) : "",
        memberIds: Array.isArray(department.members) ? department.members.map((m) => String(m.id)) : [],
      });
    }
  }, [department]);

  const selectedMembers = useMemo(() => form.memberIds.map((mid) => ({ id: mid, user: userMap.get(String(mid)) || null })).filter((x) => x.user), [form.memberIds, userMap]);
  ...
  const saveBasics = async () => {
    ...
    try {
      ...
      await patchDepartmentMutation({ id: departmentId, name: form.name.trim(), description: form.description, attendanceRadius: Number(form.attendanceRadius || 25), isActive: form.isActive }).unwrap();
      setMessage('Department updated'); await refetch();
  ...
  const saveLeader = async () => {
    try {
      ...
      await patchDepartmentMutation({ id: departmentId, leaderId: form.leaderId ? Number(form.leaderId) : null }).unwrap();
      setMessage('Department leader updated'); await refetch();
  ...
  const saveMembers = async () => {
    try {
      ...
      await patchDepartmentMutation({ id: departmentId, memberIds: form.memberIds.map(Number) }).unwrap();
  ...
  const deleteDepartment = () => {
    Alert.alert('Delete Department', 'Are you sure? This cannot be undone.', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Delete', style: 'destructive', onPress: async () => {
          try {
            setDeleting(true); setError('');
            await deleteDepartmentMutation(departmentId).unwrap();
            router.back();
  ...
  if (isLoading || isFetching) {
    return (
      <View className="flex-1 bg-slate-50 dark:bg-slate-950 p-5 items-center justify-center">
        <ActivityIndicator size="large" color="#2563eb" />
        <Text className="mt-4 text-slate-500 font-medium">Loading department details...</Text>
      </View>
    );
  }
    ...
    <View className="flex-1 bg-slate-50 dark:bg-slate-950">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 16, paddingBottom: 100 }}
        ...
        <View className="bg-white dark:bg-slate-900/80 rounded-3xl border border-slate-200 dark:border-slate-800 p-5 mb-4">
          <Text className="text-xs font-black uppercase tracking-widest text-slate-500 mb-4">Department Details</Text>
          <View className="gap-3">
            <View className="gap-1.5">
              <Text className="text-[11px] font-black uppercase tracking-widest text-slate-500">Department Name</Text>
              <TextInput value={form.name} onChangeText={(v) => setForm((p) => ({ ...p, name: v }))} editable={canUpdateDepartments}
            ...
            <View className="gap-1.5">
              ...
              <TextInput value={form.description} onChangeText={(v) => setForm((p) => ({ ...p, description: v }))} editable={canUpdateDepartments}
                multiline numberOfLines={3}
                className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-3 text-sm font-semibold text-slate-900 dark:text-white" />
            </View>
            <View className="gap-1.5">
              <Text className="text-[11px] font-black uppercase tracking-widest text-slate-500">Attendance Radius (m)</Text>
              <TextInput value={form.attendanceRadius} onChangeText={(v) => setForm((p) => ({ ...p, attendanceRadius: v.replace(/[^\d]/g, "") }))} editable={canUpdateDepartments} keyboardType="number-pad"
                className="bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-3 text-sm font-semibold text-slate-900 dark:text-white" />
            </View>
            <View className="flex-row items-center justify-between bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-3">
              ...
              <Switch value={form.isActive} onValueChange={(v) => setForm((p) => ({ ...p, isActive: v }))} disabled={!canUpdateDepartments} trackColor={{ false: "#e2e8f0", true: "#2563eb" }} thumbColor="#fff" />
            </View>
            <Pressable onPress={saveBasics} disabled={!canUpdateDepartments || savingBasics}
        ...
        {canAssignMembers && (
          <View className="bg-white dark:bg-slate-900/80 rounded-3xl border border-slate-200 dark:border-slate-800 p-5 mb-4">
            <Text className="text-xs font-black uppercase tracking-widest text-slate-500 mb-4">Department Leader</Text>
        ...
        {canDeleteDepartments && (
          <Pressable onPress={deleteDepartment} disabled={deleting}
            ...
            <Text className="text-white text-sm font-bold">Delete Department</Text>
          </Pressable>
        )}
      </ScrollView>
