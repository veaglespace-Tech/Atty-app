...
import { ChevronLeft, MapPinned, Timer, FileWarning, CalendarClock } from "lucide-react-native";
import { useAuthSession } from "@/hooks/useAuthSession";
import { useGetMyAttendanceQuery, useGetAttendanceSummaryQuery, usePunchInMutation, usePunchOutMutation, useRequestRegularizationMutation } from "@/services/api/attendanceApi";
...
export default function OrgMyAttendancePage() {
  ...
  } = useGetMyAttendanceQuery(30, { skip: !user });
  ...
  const summary = useMemo(() => Array.isArray(summaryData) ? summaryData : [], [summaryData]);
    ...
    <View className="flex-1 bg-slate-50 dark:bg-slate-950">
      <View className="px-5 pt-4 pb-4 bg-white dark:bg-[#020617] border-b border-slate-200 dark:border-slate-800">
        <View className="flex-row items-center justify-between">
          <Text className="text-xl font-black tracking-tight text-slate-900 dark:text-white">My Attendance</Text>
        </View>
      </View>
        ...
        {records.length === 0 ? (
          ...
          <View className="gap-3">
            {records.slice(0, 15).map((record) => (
              <View key={record.id} className="bg-white dark:bg-slate-900/80 rounded-[28px] border border-slate-200 dark:border-slate-800 p-5 shadow-sm">
