    ...
    <View className="flex-1 bg-slate-50 dark:bg-slate-950">
      ...
      <ScrollView className="flex-1" contentContainerStyle={{ padding: 24, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
        ...
        {!isSuperAdmin && !!referralCode && (
          <View className="bg-slate-900 dark:bg-[#0f172a] rounded-[24px] p-6 shadow-sm border border-slate-800 mb-6">
            <View className="flex-row items-center gap-3 mb-4">
              <View className="h-10 w-10 items-center justify-center rounded-xl bg-blue-500/20">
                <Link2 size={18} className="text-blue-400" />
              </View>
              <View className="flex-1">
                <Text className="text-base font-black text-white">Referral Link</Text>
                <Text className="text-xs font-semibold text-slate-400">Invite members to your workspace</Text>
              </View>
            </View>
            <View className="flex-col gap-4">
              <View>
                <Text className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5">Referral Code</Text>
                <View className="flex-row items-center bg-slate-800/80 dark:bg-slate-900/80 border border-slate-700/50 rounded-2xl p-2 pl-4">
                  <Text className="flex-1 text-sm font-bold text-slate-100" numberOfLines={1}>
                    {referralCode}
                  </Text>
                  <Pressable 
                    onPress={copyCodeToClipboard}
                    className="flex-row items-center gap-2 bg-slate-700 dark:bg-slate-800 px-4 py-2.5 rounded-xl active:scale-95 transition-transform"
                  >
                    {copiedReferralCode ? (
                      <>
                        <CheckCircle2 size={14} color="#34d399" />
                        <Text className="text-xs font-bold text-emerald-400">Copied</Text>
                      </>
                    ) : (
                      <>
                        <Copy size={14} color="#94a3b8" />
                        <Text className="text-xs font-bold text-slate-300">Copy Code</Text>
                      </>
                    )}
                  </Pressable>
                </View>
              ...
              <View>
                ...
                <View className="flex-row items-center bg-slate-800/80 dark:bg-slate-900/80 border border-slate-700/50 rounded-2xl p-2 pl-4">
                  <Text className="flex-1 text-sm font-medium text-slate-300" numberOfLines={1} ellipsizeMode="tail">
                    {referralLink}
                  </Text>
                  <Pressable 
                  ...
                  >
                    {copiedReferral ? (
                      ...
                      <>
                        <Copy size={14} color="#94a3b8" />
                        <Text className="text-xs font-bold text-slate-300">Copy Link</Text>
                      </>
                    )}
                  </Pressable>
                </View>
