...
import { ChevronLeft, UserPlus, Clock, CheckCircle2, XCircle, ShieldCheck, FileWarning } from "lucide-react-native";
...
export default function OrgRequestsPage() {
  ...
  const attItems = Array.isArray(attData?.data) ? attData.data.filter((r: any) => r.status === "PENDING") : [];
...
function RegistrationRequestsTab({ items, loading, refetch }: any) {
...
function AttendanceRequestsTab({ items, loading, refetch }: any) {
  const [actionId, setActionId] = useState("");
