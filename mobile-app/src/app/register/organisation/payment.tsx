...
import { View, Text, ScrollView, TextInput, Pressable, ActivityIndicator, Alert, Modal, SafeAreaView } from "react-native";
...
const PLAN_CODE_BY_NAME: Record<string, string> = { basic: "BASIC", pro: "PRO", advanced: "ADVANCED" };
...
const getApiErrorMessage = (error: any, fallback = "Unable to process request.") => {
...
export default function PaymentPage() {
  ...
  useEffect(() => {
    ...
    if (!org) { router.replace("/register/organisation" as any); return; }
    const admin = getRegistrationDraft(REGISTRATION_DRAFT_KEYS.admin);
    if (!admin) { router.replace("/register/organisation/admin" as any); return; }
    const plan = getRegistrationDraft(REGISTRATION_DRAFT_KEYS.selectedPlan);
    if (!plan) { router.replace("/register/organisation/plan" as any); return; }
    
    const normalized = normalizePlan(plan);
    if (!normalized?.code || isHiddenPaidMonthlyPlan(normalized)) {
      clearRegistrationDraft(REGISTRATION_DRAFT_KEYS.selectedPlan);
      router.replace("/register/organisation/plan" as any);
      return;
    }
  ...
  const finalizeSuccess = ({ organization, admin, plan, emailSent, freeTrial = false }: any) => {
    setSuccessState({ 
  ...
  const handlePayment = async () => {
    if (!selectedPlan) {
      ...
      router.push("/register/organisation/plan" as any);
      return;
    }
    const organization = getRegistrationDraft(REGISTRATION_DRAFT_KEYS.organisation);
    const admin = getRegistrationDraft(REGISTRATION_DRAFT_KEYS.admin);
    if (!organization || !admin) {
      ...
      router.push("/register/organisation" as any);
      return;
    }
    ...
    try {
      const payload: any = { planCode: selectedPlan.code, organization, admin };
      if (appliedCoupon) {
      ...
      {successState ? (
        <View className="bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm mt-4">
          ...
          <View className="gap-3">
            <Pressable
              onPress={() => { clearAllRegistrationDrafts(); router.push("/login" as any); }}
            ...
            <Pressable
              onPress={() => { clearAllRegistrationDrafts(); router.push("/" as any); }}
        ...
        <View>
          ...
          <View className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-100 dark:border-slate-800 shadow-sm">
            ...
            <View className="gap-3 mb-6">
              {(selectedPlan?.features || ["Core Platform Features", "Advanced Location & Attendance", "Admin Role Controls", "Premium Support"]).slice(0, 4).map((f: string, i: number) => (
