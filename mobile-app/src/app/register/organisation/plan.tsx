...
export default function Plans() {
  ...
  const [selectedDurationOverride, setSelectedDurationOverride] = useState<number | null>(null);
  
  const storedSelectedPlan = useSyncExternalStore(
    emptySubscribe,
    () => getRegistrationDraftRaw(REGISTRATION_DRAFT_KEYS.selectedPlan) as string | null,
    () => null
  );
  ...
  useEffect(() => {
    ...
    if (!storedOrganization) {
      router.replace("/register/organisation" as any);
      return;
    }

    if (!storedAdmin) {
      router.replace("/register/organisation/admin" as any);
    }
  }, [router]);
  ...
  const tiers = useMemo(() => {
    ...
    return Object.values(grouped).map((tier: any) => {
      tier.options.sort((a: any, b: any) => a.durationInDays - b.durationInDays);
      return tier;
    });
  }, [rawPlans]);
  ...
  const handleSelectPlan = (plan: any) => {
    ...
    router.push("/register/organisation/payment" as any);
  };
      ...
      <View className="gap-6">
        {tiers.map((tier: any) => {
          ...
          const planOption = isFreeTier
            ? tier.options[0]
            : tier.options.find((option: any) => option.durationInDays === selectedDuration) || tier.options[0];
          ...
          return (
            <View
              ...
              className={`bg-white dark:bg-slate-900 rounded-3xl p-6 border ${
                tier.popular ? "border-blue-200 dark:border-blue-500/30" : "border-slate-100 dark:border-white/10"
              } shadow-sm`}
            >
