  ...
  return (
    ...
    <SafeAreaView className="flex-1 bg-slate-100 dark:bg-slate-950">
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} className="flex-1">
        <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: 40 }} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="always">
          {/* Header */}
          <View className="px-5 pt-4 pb-4 flex-row items-center bg-slate-100 dark:bg-slate-950">
            <TouchableOpacity onPress={() => router.canGoBack() ? router.back() : router.replace('/super-admin/dashboard')} className="h-10 w-10 rounded-full bg-white dark:bg-slate-900/80 items-center justify-center shadow-sm border border-slate-200 dark:border-slate-800 mr-4 active:opacity-70">
              <ArrowLeft size={20} className="text-slate-700 dark:text-white" />            </TouchableOpacity>
            ...
            <View className="bg-white dark:bg-slate-900/80 rounded-[28px] p-5 shadow-sm border border-slate-200 dark:border-slate-800 mb-6 flex-row items-center">
              <View className="relative">
                {user?.profileImageUrl ? (
                  <Image source={{ uri: user.profileImageUrl }} resizeMode="contain" className="h-20 w-20 rounded-2xl border-2 border-slate-100 dark:border-slate-800 bg-white" />
                ) : (
                  <View className="h-20 w-20 rounded-2xl bg-blue-100 dark:bg-blue-900/40 items-center justify-center border border-blue-50 dark:border-blue-900">
                ...
                {completionState.percentage === 100 ? (
                  <View className="absolute -bottom-1 -right-1 h-6 w-6 rounded-full bg-blue-500 border-2 border-white dark:border-slate-900 items-center justify-center">
