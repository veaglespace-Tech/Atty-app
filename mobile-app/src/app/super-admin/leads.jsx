...
import { View, Text, Pressable, ScrollView, RefreshControl, ActivityIndicator, Alert, TextInput, Modal, Platform } from "react-native";
...
export default function LeadsPage() {
  ...
  const handleBack = () => {
    try {
      ...
      // In a Tab Navigator with history, 'history' array exists and contains visited keys.
      // In a Stack, 'index' > 0 means we can go back.
      if ((state?.history && state.history.length > 1) || (state?.index > 0)) {
  ...
  const handleDelete = (id, name) => {
    if (Platform.OS === 'web') {
      if (window.confirm(`Are you sure you want to delete lead '${name}'?`)) {
        deleteLead(id).unwrap().catch(err => 
