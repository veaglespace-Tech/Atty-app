          ...
          >
            {isLoading ? (
              ...
              <View className="pb-10">
                <Text className="text-xs font-black uppercase tracking-widest text-slate-400 mb-4 ml-1">
                  Team Members ({members.length})
                </Text>
                {members.map((member) => (
                  <View
                    key={member.id}
                    className="flex-row items-center bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl mb-3 border border-slate-100 dark:border-slate-800/60"
                  >
                    ...
                    {member.id === team?.leaderId && (
                      <View className="px-2 py-1 bg-amber-100 dark:bg-amber-900/30 rounded-md">
                        <Text className="text-[10px] font-black uppercase tracking-wider text-amber-700 dark:text-amber-500">
                          Leader
                        </Text>
                      </View>
                    )}
            ...
            >
              <View className="flex-row items-start justify-between mb-5">
                <View className="flex-1 pr-4">
                  <Text className="text-xl font-black text-slate-900 dark:text-white leading-tight mb-1">
                    {team.name}
                  </Text>
                ...
                <View
                  className={`px-3 py-1.5 rounded-full ${team.isActive ? "bg-emerald-100 dark:bg-emerald-500/20" : "bg-slate-200 dark:bg-slate-800"}`}
                >
                  <Text
                    className={`text-[10px] font-black uppercase tracking-widest ${team.isActive ? "text-emerald-700 dark:text-emerald-400" : "text-slate-600 dark:text-slate-400"}`}
                  >
