  ...
  return (
    <View className="flex-1 bg-slate-50 dark:bg-slate-950">
      <FlatList
        ...
        ListHeaderComponent={
          <View className="mb-6">
            <Text className="text-2xl font-black text-slate-900 dark:text-white">My Assigned Instruments</Text>
        ...
        ListEmptyComponent={
          isError ? (
            ...
            <View className="py-12 items-center justify-center bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-sm mt-4">
              ...
              <Text className="text-lg font-bold text-slate-900 dark:text-white mb-2">No Instruments Assigned</Text>
          ...
          <View className="bg-white dark:bg-slate-900 p-5 rounded-[20px] mb-4 border border-slate-200 dark:border-slate-800 shadow-sm">
            <View className="flex-row items-start gap-4">
              ...
              <View className="flex-1">
                <Text className="text-lg font-bold text-slate-900 dark:text-white mb-1">{inst.name}</Text>
                {inst.description ? (
                  <Text className="text-sm text-slate-500 dark:text-slate-400 mb-4">{inst.description}</Text>
                ) : <View className="mb-4" />}
                ...
                <View className="flex-row flex-wrap gap-2 border-t border-slate-100 dark:border-slate-800 pt-4">
                  {inst.assetId ? (
                    <View className="flex-row items-center bg-slate-50 dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-100 dark:border-slate-700">
                      ...
                      <Text className="text-xs font-bold text-slate-700 dark:text-slate-300">ID: {inst.assetId}</Text>
                    </View>
                  ) : null}
                  ...
                  <View className="flex-row items-center bg-slate-50 dark:bg-slate-800 px-3 py-1.5 rounded-lg border border-slate-100 dark:border-slate-700">
                    ...
                    <Text className="text-xs font-bold text-slate-700 dark:text-slate-300">
                      Assigned {new Date(inst.assignedAt || Date.now()).toLocaleDateString()}
                    </Text>
                  </View>
                </View>
              </View>
