...
export default function SettingsScreen() {
  ...
  const APP_URL =
    process.env.EXPO_PUBLIC_APP_URL || "https://attendee.veaglespace.com";
  const referralLink = referralCode
    ? `${APP_URL}/register/user?ref=${referralCode}`
    : "";
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (
        ...
        getLocalPhoneNumber(user?.emergencyContact)
      ) {
        payload.emergencyContactCountryCode = "+91";
        payload.emergencyContact = formatPhoneNumberForSave(
          formData.emergencyContact,
        );
      }
      if (formData.currentAddress !== user?.currentAddress)
        payload.currentAddress = formData.currentAddress;
      if (formData.permanentAddress !== user?.permanentAddress)
      ...
      if (formData.physicalFormNo !== (user?.physicalFormNo || ""))
        payload.physicalFormNo = formData.physicalFormNo || undefined;
      if (formData.bloodGroup !== (user?.bloodGroup || ""))
        payload.bloodGroup = formData.bloodGroup || undefined;
      if (formData.gender !== (user?.gender || ""))
        payload.gender = formData.gender || undefined;
      if (formData.existingMember !== (user?.existingMember || ""))
        payload.existingMember = formData.existingMember || undefined;
      if (
  ...
  const currentProfileImageUrl =
    profileImageDataUrl ||
    (user?.profileImageUrl ? getFullImageUrl(user.profileImageUrl) : null);
      ...
      >
        <View className="max-w-2xl w-full mx-auto">
            ...
            >
              {currentProfileImageUrl ? (
                <Image
                  ...
                  resizeMode="contain"
                  className="h-24 w-24 rounded-2xl border-4 border-white dark:border-slate-800 bg-white"
                />
              ) : (
          ...
          <View className="flex-row mb-6 bg-slate-200/50 dark:bg-slate-900/50 p-1.5 rounded-2xl border border-slate-200 dark:border-slate-800">
            ...
            >
              <Text
                className={`text-sm font-bold ${activeTab === "personal" ? "text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-400"}`}
              >
                Personal
              </Text>
            </Pressable>
            {canSeeAdminSettings && (
              ...
              >
                <Text
                  className={`text-sm font-bold ${activeTab === "organization" ? "text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-400"}`}
                >
                  Workspace
                </Text>
              </Pressable>
            ...
            >
              <Text
                className={`text-sm font-bold ${activeTab === "security" ? "text-slate-900 dark:text-white" : "text-slate-500 dark:text-slate-400"}`}
              >
                Security
              </Text>
            </Pressable>
                ...
                <View className="flex-row items-center gap-4">
                  ...
                  <View>
                    ...
                    <Text className="text-xs font-semibold text-slate-500">
                      Create departments and allocate users
                    </Text>
                  </View>
                </View>
              ...
              <View className="gap-y-4">
                ...
                <View>
                  ...
                  <View className="flex-row items-center bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-3.5">
                    ...
                    <Text className="text-sm font-semibold text-slate-900 dark:text-white mr-1">
                      +91
                    </Text>
                    <TextInput
                  ...
                  <View className="flex-row items-center bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-3.5">
                    ...
                    <TextInput
                      ...
                      onChangeText={(val) =>
                        setFormData((prev) => ({
                          ...prev,
                          emergencyContact: val.replace(/[^0-9]/g, ""),
                        }))
                      }
                      className="flex-1 text-sm font-semibold text-slate-900 dark:text-white"
                  ...
                  {Platform.OS === "web" ? (
                    ...
                    <>
                      ...
                      >
                        ...
                        />
                        <Text
                          className={`flex-1 text-[14px] font-bold ${formData.dob ? "text-slate-900 dark:text-white" : "text-slate-400 dark:text-slate-500"}`}
                        >
                ...
                <View>
                  <Text className="text-xs font-bold uppercase tracking-wider text-slate-500 ml-1 mb-1.5">
                    Document Uploader (Aadhar, PAN, etc.)
                  </Text>
                  <View className="bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 rounded-2xl p-4">
                    <Text className="text-xs text-slate-500 dark:text-slate-400 mb-4">
                      Upload your identity or reference document (PDF, PNG,
                    ...
                    <View className="flex-row flex-wrap gap-2 mb-3">
                      <Pressable
                        ...
                        className="flex-row items-center gap-2 bg-blue-50 dark:bg-blue-500/10 border border-blue-200 dark:border-blue-500/30 rounded-xl px-4 py-3"
                      >
                        <FileText
                          size={16}
                          className="text-blue-600 dark:text-blue-400"
                        ...
                        <Text className="text-sm font-semibold text-blue-700 dark:text-blue-300">
                          {documentDataUrl || user?.documentUrl
                            ? "Replace Document"
                            : "Upload Document"}
                        </Text>
                      </Pressable>
                      ...
                      {(removeDocument ||
                        ...
                        user?.documentUrl) && (
                        <Pressable
                          onPress={toggleDocumentRemoval}
                          className="flex-row items-center gap-2 bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/30 rounded-xl px-4 py-3"
                        >
                          <Trash2
                            size={16}
                      ...
                      {user?.documentUrl &&
                        ...
                        !documentDataUrl && (
                          <Pressable
                            onPress={() =>
                              WebBrowser.openBrowserAsync(user.documentUrl)
                            }
                            className="flex-row items-center gap-2 bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-xl px-4 py-3"
                          >
                            <Link2
                              size={16}
                    ...
                    <View className="bg-white dark:bg-slate-950 px-3 py-2.5 rounded-xl border border-slate-100 dark:border-slate-800">
                      <Text className="text-[11px] font-medium text-slate-500 dark:text-slate-400">
                        {removeDocument
                          ? "Your document will be removed when you save."
                          : documentDataUrl
                            ? `Selected: ${documentName}. Click Save Changes.`
                            : user?.documentUrl
                              ? `Current: ${user.documentName || "Uploaded File"}`
          ...
          {activeTab === "personal" && (
            <View className="bg-white dark:bg-slate-900 rounded-[24px] p-6 mb-6 shadow-sm border border-slate-200 dark:border-slate-800">
              ...
              <View className="flex-row items-center gap-4 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-2xl border border-slate-100 dark:border-slate-800 mb-3">
                ...
                <View>
                  <Text className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-0.5">
                    Email Support
                  </Text>
                  ...
                  <View className="flex-col gap-4">
                    <View>
                      ...
                      <View className="flex-row items-center bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700/50 rounded-2xl p-2 pl-4">
                        <Text
                          className="flex-1 text-sm font-bold text-slate-900 dark:text-slate-100"
                          numberOfLines={1}
                        >
                          {referralCode}
                        </Text>
