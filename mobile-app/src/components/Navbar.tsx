...
import { LayoutDashboard, LogIn, UserPlus, Menu, X, ChevronRight, LogOut, UserCircle2 } from "lucide-react-native";
...
// import ThemeToggle from "@/components/ThemeToggle"; // TODO: Implement ThemeToggle for React Native
          ...
          <View className="flex-row h-16 items-center justify-between">
            ...
            <View className="hidden md:flex-row items-center gap-3">
              ...
              {!isAuthReady ? (
                ...
                <>
                  <Link href={dashboardHref as any} asChild>
        ...
        <View className="absolute bottom-0 top-16 left-0 right-0 bg-white dark:bg-slate-950 p-4 shadow-xl">
          <ScrollView contentContainerStyle={{ gap: 24 }}>
            <View className="gap-4">
              {NAV_LINKS.map((link) => (
                <Link key={link.href} href={link.href as any} asChild onPress={closeMenu}>
                  <Pressable
            ...
            <View className="rounded-[28px] border border-slate-200 bg-white p-4 dark:border-slate-800 dark:bg-slate-900">
              {!isAuthReady ? (
                ...
                <>
                  ...
                  <Link href={dashboardHref as any} asChild onPress={closeMenu}>
...
const PublicNavLink = memo(function PublicNavLink({ href, label, active }: { href: any; label: string; active: boolean }) {
  return (
