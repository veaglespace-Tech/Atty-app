...
export default function PricingPage() {
  ...
  const formatDurationShort = (days) => {
    if (days === 7) return "7D";
  ...
  const tiers = useMemo(() => {
    ...
    const grouped = rawPlans.reduce((accumulator, plan) => {
    ...
    <div className="relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-100 via-white to-blue-50 px-4 pb-24 pt-32 transition-colors duration-500 dark:from-slate-950 dark:via-slate-950 dark:to-slate-900">
      ...
      <div className="relative z-10 mx-auto max-w-6xl">
              ...
              return (
                ...
                >
                  <div className="flex h-full flex-col rounded-[2.3rem] border border-slate-100 bg-white p-8 transition-all duration-500 dark:border-slate-800 dark:bg-slate-950/90 md:p-10">
                    {tier.popular ? (
                      <div className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 rounded-full bg-blue-600 px-4 py-1.5 text-[10px] font-black uppercase tracking-widest text-white shadow-xl shadow-blue-200 dark:bg-blue-400 dark:text-slate-950">
                        Most Popular
                      </div>
                    ) : null}
                    ...
                    <div className="mb-6 flex items-baseline gap-1">
                      <AnimatePresence mode="wait">
                        <motion.span
                          ...
                          animate={{ opacity: 1, y: 0 }}
                          exit={{ opacity: 0, y: -10 }}
                          className="text-4xl font-black text-slate-950 dark:text-white"
                        >
                          Rs. {formatPrice(selectedPlan.price)}
                        </motion.span>
                      </AnimatePresence>
                      <span className="text-sm font-bold text-slate-500 dark:text-slate-400">
                        /{formatDurationLong(selectedPlan.durationInDays)}
                      </span>
                    </div>
                    ...
                    <div className="mb-8 flex rounded-2xl border border-slate-200 bg-slate-100/80 p-1.5 dark:border-slate-800 dark:bg-slate-900/80">
                      {tier.options.map((option) => {
                        ...
                        return (
                          ...
                          >
                            {formatDurationShort(option.durationInDays)}
                          </button>
                        );
                      })}
                    </div>
        ...
        <motion.div
          ...
          className="relative mt-20 overflow-hidden rounded-[3rem] bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 p-10 text-white shadow-[0_34px_96px_rgba(15,23,42,0.18)] dark:shadow-blue-950/25"
        >
          <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/cubes.png')] opacity-10" />
          <div className="relative z-10 flex flex-col items-center justify-between gap-8 md:flex-row">
            <div className="max-w-md">
              <h4 className="mb-3 text-3xl font-black text-white">Need a Custom Solution?</h4>
              <p className="text-lg font-medium leading-relaxed text-slate-300 dark:text-slate-400">
            ...
            <Link
              href="/contact"
              className="shrink-0 rounded-3xl bg-white px-10 py-5 text-sm font-black uppercase tracking-widest text-slate-900 shadow-2xl transition-all hover:-translate-y-0.5 hover:bg-blue-50 active:scale-95"
            >
              Contact Enterprise
            </Link>
          </div>
