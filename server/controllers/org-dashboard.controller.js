...
exports.downloadOrgReportsPdf = asyncHandler(async (req, res) => {
  ...
  const pdfBuffer = await buildGenericTablePdf({
    ...
    columns: [
      ...
      { key: "member", label: "Member", width: 90 },
      { key: "role", label: "Role", width: 70 },
      { key: "gender", label: "Gender", width: 50 },
      { key: "existingMember", label: "Type", width: 60 },
      { key: "bloodGroup", label: "Blood", width: 45 },
      { key: "referenceBy", label: "Ref By", width: 70 },
      { key: "emergencyContact", label: "Emergency", width: 80 },
      { key: "joinedAt", label: "Joined", width: 65 },
      ...
      { key: "workedHours", label: "Worked", width: 60, align: "center" },
    ],
