...
exports.downloadOrgAttendancePdf = asyncHandler(async (req, res) => {
  ...
  const pdfBuffer = await buildGenericTablePdf({
    ...
    columns: [
      ...
      { key: "user", label: "Member", width: 90 },
      ...
      { key: "punchIn", label: "Punch In", width: 60, align: "center" },
      { key: "punchOut", label: "Punch Out", width: 60, align: "center" },
      { key: "reachedHome", label: "R. Home", width: 60, align: "center" },
      { key: "reachedHomeLocation", label: "R. Home Loc", width: 80 },
    ...
    rows: payload.items.map((item, index) => {
      return {
        ...
        departmentStatus: item.departmentStatus || "Unallocated",
        gender: item.gender || "-",
...
exports.downloadOrgAttendanceExcel = asyncHandler(async (req, res) => {
  ...
  const excelBuffer = await buildExportWorkbookBuffer({
    ...
    columns: [
      ...
      { key: "departmentStatus", label: "Allocation Status", width: 110 },
      { key: "gender", label: "Gender", width: 70 },
      { key: "dob", label: "Date of Birth", width: 85 },
